      ...
      <div className="p-10">
        <div className="flex flex-col md:flex-row justify-between max-w-full mx-auto">
          <div className="w-full md:w-1/2 mb-6 md:mb-0 flex items-center min-h-[200px]">
            <div>
              <h3 className="text-base mb-4">{typedFooterData.getInTouch.subtitle}</h3>
              {typedFooterData.getInTouch.headings.map((heading, index) => (
                <h1 key={index} className="text-6xl">{heading}</h1>
              ))}
              <p className="text-white text-base mt-10">{typedFooterData.getInTouch.description}</p>
            </div>
          </div>
          <div className="w-full md:w-1/3 bg-white p-12 rounded-lg shadow-md">
            <h3 className="text-3xl text-black font-bold mb-4">{typedFooterData.contactForm.title}</h3>
            <p className="text-base text-black">{typedFooterData.contactForm.description}</p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6 md:gap-20 mt-10">
          ...
          <div>
            ...
            <p className="text-base text-white">{typedFooterData.logoSection.description}</p>
          </div>

          {/* Column 2: Call Center */}
          <div>
            <h4 className="text-lg font-semibold mb-3">{typedFooterData.callCenter.title}</h4>
            <ul className="space-y-2 text-base text-white">
              ...
              <li className="mt-10">{typedFooterData.callCenter.email.label}</li>
          ...
          {/* Column 3: Location */}
          <div>
            <h4 className="text-lg font-semibold mb-3">{typedFooterData.location.title}</h4>
            <ul className="space-y-2 text-base text-white">
              {typedFooterData.location.addresses.map((address, index) => (
                <li key={index}>{address}</li>
              ))}
              <li className="mt-10">{typedFooterData.location.socialMedia.label}</li>
              <li>
                <div className="flex items-center gap-6 text-white text-base mt-3">
                  {typedFooterData.location.socialMedia.icons.map((icon, index) => {
          ...
          {/* Column 5: Services */}
          <div>
            <h4 className="text-lg font-semibold mb-3">{typedFooterData.services.title}</h4>
            <ul className="space-y-2 text-base text-gray-300">
