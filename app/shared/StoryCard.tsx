...
const Card: React.FC<StoryCardProps> = ({ heading, stars, content }) => {
  const starCount = Array.isArray(stars) ? Math.min(stars.length, 5) : Math.min(stars, 5);
  const starsArray = Array.from({ length: starCount }, (_, index) => index);

  return (
    <div className="bg-white shadow-lg rounded-lg p-5" style={{ minHeight: "0", maxHeight: "none" }}>
